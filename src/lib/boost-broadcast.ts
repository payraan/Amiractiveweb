...
export async function broadcastBoostedMarket(
  ...
  // ⚠️ **زنجیره راه می‌افتد، نه یک تیک تنها.**
  //
  // نسخه‌ی قبلی اینجا یک `runBroadcastTick(5_000)` می‌زد و تمام. نتیجه‌اش
  // این بود که بوست — تنها قابلیتی که کاربر بابتش پول واقعی می‌دهد —
  // کندترین مسیر پخش را داشت: بعد از آن پنج ثانیه، ادامه‌اش می‌افتاد گردن
  // کرون هر ۱۵ دقیقه با بودجه‌ی ۱۰ ثانیه، یعنی حدود ۲۵۰ پیام در ربع
  // ساعت. در همان حال، پخش سراسری ادمین از راه `/api/bot/broadcast`
  // خودش را زنجیر می‌کرد و با ۲۵ پیام در ثانیه می‌رفت.
  //
  // حالا هر دو یک مسیر دارند. با مخاطب کوچک تفاوتی دیده نمی‌شود؛ با ده
  // هزار نفر، تفاوت بین چند دقیقه و بیش از یک روز است.
  //
  // اگر پیکربندی زنجیره ناقص بود، همان تیک درجای قبلی زده می‌شود تا
  // دست‌کم چیزی برود و کرون بقیه را ببرد.
  if (!kickBroadcastChain()) {
    try {
