// چالش پراپ پیش‌بینی — ورود با MOON، سنجش با پوینت، جایزه‌ی مهارتی.
...
// فقط پیش‌بینی‌هایی که در این بازه‌ی احتمال ثبت شده‌اند در ارزیابی چالش
...
// سقف تعداد ورود به چالش در ۳۰ روز گذشته (فقط مسیر حساب پیش‌بینی).
...
export async function startChallenge(
  ...
  try {
    ...
    // ردیف بازیکن را همین اول قفل می‌کنیم. هر بررسیِ بعدی (چالش فعال و
...
export type ChallengeState = {
  ...
  daysLeft: number;
  prize: string;
  /** کارنامه‌ی برد و باخت داخل همین چالش */
  wins: number;
  losses: number;
  winRate: number | null;
  bestDay: number;
