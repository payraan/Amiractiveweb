...
export function marketPoll(
  ...
  if (mode === "live") {
    ...
    return {
      text,
      buttons: [
        // رنگ‌ها همان قاعده‌ی حالت فوروارد — دو حالت یک کارت‌اند و
        // نباید یکی رنگی باشد و دیگری نه.
        [
          {
            text: `✅ بله (${m.yesPct}٪)`,
            callback_data: `${cb}:y:${m.id}`,
            style: "success" as const,
          },
          {
            text: `❌ خیر (${noPct}٪)`,
            callback_data: `${cb}:n:${m.id}`,
            style: "danger" as const,
          },
        ],
        ...(deep
          ? [[{ text: "📊 باز کردن بازار", url: deep, style: "primary" as const }]]
          : []),
      ],
    };
  }
  ...
  // است و حساب می‌سازد.
  // ⚠️ رنگ‌ها عمدی‌اند و از همان قاعده‌ی بالای این فایل می‌آیند. کارت بازار
  // تنها جایی بود که `style` نداشت، در حالی که پرمخاطب‌ترین پیام پلتفرم
  // است: همان چیزی که در کانال دیده و فوروارد می‌شود.
  //
  // بله سبز و خیر قرمز، چون این دو **دو انتخاب مقابل هم‌اند** و رنگ،
  // تصمیم را یک نگاه سریع‌تر می‌کند. مقصد اصلی آبی می‌ماند.
  //
  // دکمه‌ی ثبت‌نام عمدا بی‌رنگ است: اگر هر چهار دکمه رنگی باشند، رنگ دیگر
  // چیزی نمی‌گوید.
  const buttons: InlineButton[][] = deep
    ? [
        [
          { text: `✅ بله (${m.yesPct}٪)`, url: `${deep}_yes`, style: "success" },
          { text: `❌ خیر (${noPct}٪)`, url: `${deep}_no`, style: "danger" },
        ],
        [
          {
            text: "📊 مشاهده بازار و ثبت پیش‌بینی",
            url: deep,
            style: "primary",
          },
        ],
