...
// اندازه‌ی استخر اتصال.
//
// ۵ خیلی کم بود. تست بار نشان داد با ۳۰ درخواست همزمان روی یک بازار، هر
// شرط ۱۱۱ میلی‌ثانیه سرویس می‌گیرد در حالی که کارِ واقعیِ دیتابیس ۳
// میلی‌ثانیه است؛ بقیه‌اش انتظار پشت استخر است. چون شرط داخل ترنزاکشن
// ردیف بازار را قفل می‌کند، هر ثانیه‌ای که اتصال بی‌جهت نگه داشته شود
// مستقیم به صفِ بقیه اضافه می‌شود.
//
// ⚠️ سقفش را کورکورانه بالا نبر: هر نمونه‌ی سرور استخر خودش را دارد و
// Postgres سقف کل اتصال دارد (پیش‌فرض Railway معمولا ۱۰۰). با دو نمونه و
// استخر ۲۰، می‌شود ۴۰ — که جا دارد؛ با استخر ۱۰۰ نه.
const POOL_MAX = Math.max(1, Number(process.env.PGPOOL_MAX) || 20);
...
const pool =
  ...
  new Pool({
    ...
    max: POOL_MAX,
    // بدون این، درخواستی که اتصال گیرش نیامد **بی‌نهایت** در صف می‌ماند و
    // کاربر فقط یک صفحه‌ی هنگ‌کرده می‌بیند. با سقف، خطا می‌گیرد و لااقل
    // می‌فهمد چیزی خراب است.
