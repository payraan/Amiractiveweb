...
export function homeText(displayName: string | null): string {
  ...
  return (
    `${hi}\n\n` +
    `به نارمون خوش آمدی.\n\n` +
    `اینجا هر پیش‌بینی‌ای که ثبت می‌کنی می‌ماند و شمرده می‌شود. ` +
    `قیمت دلار و طلا، نتیجه‌ی اتفاق‌هایی که این روزها همه درباره‌شان حرف می‌زنند، ` +
    `و بازارهایی که خود کاربران می‌سازند.\n\n` +
    `در بازار ایران با تتر وارد می‌شوی و ضریب را جمعیت تعیین می‌کند، نه ما. ` +
    `در بقیه‌ی بخش‌ها فقط امتیاز در میان است و پولی درگیر نیست.\n\n` +
    `یک نکته را از همان اول بگویم: <b>امتیاز اینجا فروشی نیست.</b> ` +
    `هر مقدار پول هم بگذاری رتبه نمی‌خری، و کسی که شانسی حدس بزند ` +
...
export function guestText(): string {
  return (
    `سلام 👋\n\n` +
    `<b>نارمون</b> جایی است که پیش‌بینی‌هایت جدی گرفته می‌شود.\n\n` +
    `از قیمت دلار و طلا تا نتیجه‌ی رویدادهای واقعی، حدست را ثبت می‌کنی و ` +
    `کارنامه‌ای می‌سازی که بماند. در بازار ایران با تتر وارد می‌شوی و ` +
    ...
    `برای شروع کافی است اپلیکیشن را باز کنی؛ حسابت همان‌جا با همین ` +
