 ...
 *
 * ── قاعده‌ی رنگ ──
 * Bot API فقط سه استایل دارد: `primary` (آبی)، `success` (سبز)،
 * `danger` (قرمز). رنگ چهارمی وجود ندارد، پس هر سه باید **معنا** داشته
 * باشند وگرنه منو می‌شود تابلوی تبلیغات.
 *
 * قاعده‌ی نارمون:
 *   آبی  → مقصد اصلی (اپ و بازار پولی)
 *   سبز  → پول و رشد (کیف پول، دعوت، کانال)
 *   بی‌رنگ → بازی‌های امتیازی و ابزارها
 *
 * ⚠️ **ردیف‌ها هم‌رنگ نگه داشته می‌شوند.** نسخه‌ی قبلی شش دکمه‌ی آبی داشت و
 * دو ردیفش نیمه‌رنگی بود؛ چشم آن را به‌هم‌ریخته می‌بیند، نه سلسله‌مراتب.
 * حالا هر ردیف یا کاملا رنگی است یا کاملا خنثی، و رنگ‌ها فقط روی سه ردیف
 * اول و ردیف رشد خرج می‌شوند.
 */
export function mainKeyboard(): InlineButton[][] {
  ...
  if (SITE_URL) {
    ...
    // بازار ایران تنها جایی است که پول واقعی درگیر است و کیف پول همراهش —
    // این ردیف مقصد اصلی محصول است.
    rows.push([
      { text: "🇮🇷 بازار ایران", web_app: { url: appUrl("markets") }, style: "primary" },
      { text: "👛 کیف پول", callback_data: MENU.wallet, style: "success" },
    ]);
    // بازی‌های امتیازی: بی‌رنگ. پولی در میان نیست و نباید هم‌وزن بازار
    // ایران دیده شوند.
    rows.push([
  ...
  // ردیف رشد — هر دو سبز، چون هر دو به درآمد کاربر وصل‌اند.
  rows.push([
    { text: "🎁 دعوت دوستان", callback_data: MENU.invite, style: "success" },
    { text: "📣 کانال من", callback_data: MENU.channels, style: "success" },
  ]);
  rows.push([
...
export async function inviteScreen(playerId: number): Promise<Screen> {
  ...
  const text =
    ...
    // ⚠️ لحن عمدا **دعوت** است نه بازاریابی شبکه‌ای. دلیلِ کار، کیفیت خودِ
    // بازار است؛ سهم و پورسانت شفاف گفته می‌شود ولی حرف اول نیست.
    `<b>چرا دعوت کنی</b>\n` +
    `بازار پیش‌بینی با دو نفر معنا ندارد. هرچه آدم‌های بیشتری روی یک سؤال ` +
    `نظر بدهند، عدد نهایی به واقعیت نزدیک‌تر می‌شود — و بحث سرِ بازارها با ` +
    `کسانی که می‌شناسی، خودش نصف ماجراست.\n\n` +
    `<b>شفاف بگوییم چه اتفاقی می‌افتد</b>\n` +
    `• کسی که با لینک تو وارد شود، <b>${num(REFERRAL_BONUS)} MOON</b> هدیه‌ی ` +
    `شروع می‌گیرد.\n` +
    `• اگر روزی MOON بخرد، <b>${num(REFERRAL_PERCENT)}٪</b> آن به تو هم ` +
    `می‌رسد. از سهم او کم نمی‌شود.\n` +
    `• اگر بازاری بسازی و او رویش پیش‌بینی کند، سهم بیشتری از کارمزد همان ` +
    `بازار به تو می‌رسد.\n\n` +
    `<i>هیچ‌کدام اجباری نیست و هیچ‌کدام امتیاز یا رتبه نمی‌خرد. امتیاز فقط ` +
    `از درست پیش‌بینی‌کردن می‌آید.</i>\n\n` +
    `<b>کارنامه</b>\n` +
