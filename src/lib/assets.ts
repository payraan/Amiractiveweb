...
export function isLikelyOpen(cat: AssetCategory, now = new Date()): boolean {
  ...
  // ── سهام: فقط جلسه‌ی اصلی ───────────────────────────────────
  //
  // ⚠️ **این شرط قبلا فقط `day >= 1 && day <= 5` بود و یک حفره‌ی جدی
  // می‌ساخت.** بورس آمریکا ۱۴:۳۰ تا ۲۱:۰۰ UTC باز است، یعنی ۱۷ ساعت از
  // هر روز کاری بسته. `regularMarketPrice` در آن ساعت‌ها روی آخرین
  // بسته‌شدن **یخ می‌زند** و تکان نمی‌خورد.
  //
  // پیامدش: کاربر همان قیمت نمایش‌داده‌شده را وارد می‌کرد، یک ساعت بعد
  // قیمت تسویه دقیقا همان بود، خطا صفر می‌شد و **۱۰۰ امتیاز تضمینی**
  // می‌گرفت. با سقف ۶۰ پیش‌بینی لیدربورد، ۲۴۰ MOON (حدود ۱۵ تا ۲۵ دلار)
  // کافی بود تا کسی صدر جدول را بخرد — دقیقا همان چیزی که کل امتیازدهی
  // صفر-انتظار برای جلوگیری‌اش ساخته شده.
  //
  // ۱۴:۳۰ عمدا به ۱۵:۰۰ گرد شده: نیم‌ساعت اول را از دست می‌دهیم، ولی
  // هیچ دقیقه‌ای از ساعت‌های بسته باز نمی‌ماند. مرزِ محافظه‌کار، سمتِ
  // بسته‌بودن.
  if (cat === "stock") {
