...
// یعنی پیش‌بینی‌های تصادفی و بدون تحلیل ضرر است، و چون هر پیش‌بینی اضافه به‌طور میانگین
...
// ── چرا گیره‌ها این‌قدر بازند ────────────────────────────────
//
// ⚠️ گیره‌ی پایین قبلا ۰.۲ بود و **در عمل روی جفت‌ارزها می‌گرفت**: نوسان
// واقعی EURUSD حدود ۰.۲۶٪ است، یعنی ضریب ۰.۱۳ — زیر گیره. آستانه‌ها
// به‌جای ۰.۱۳ روی ۰.۲ اعمال می‌شد، یعنی حدود ۵۰٪ گشادتر از آنچه باید،
// و امید امتیازِ بازیکن بی‌مهارت روی جفت‌ارز از ۴.۶− می‌رفت به حدود ۷+.
// هر هشت جفت‌ارز فهرست، مزرعه‌ی امتیاز بودند.
//
// ⚠️ گیره‌ی بالا قبلا ۴.۰ بود و برعکس عمل می‌کرد: دارایی با نوسان بالای
// ۸٪ آستانه‌های تنگ‌تر از حقش می‌گرفت و امید امتیازش تا ۴۴− می‌رفت —
// تله‌ای که هیچ مهارتی نجاتش نمی‌داد.
//
// حالا هر دو آن‌قدر بازند که فقط جلوی داده‌ی خراب را بگیرند، نه جلوی
// دارایی سالم. سنجش با داده‌ی واقعی: AAPL ۰.۵۷ · BTC ۰.۷۳ · EURUSD
// ۰.۱۳ · طلا ۰.۵۷ — هیچ‌کدام دیگر به گیره نمی‌خورند.
const VOL_SCALE_MIN = 0.05;
const VOL_SCALE_MAX = 8.0;
...
// اضافه شدند چون چالش‌های بزرگ تا ۲۸۰۰ MOON ورودی دارند و کاربر نباید
