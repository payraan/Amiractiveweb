...
async function handleStart(chatId: number, player: Player | null) {
  // ── چرا این پیامِ حامل ساخته و بعد پاک می‌شود ──────────────
  //
  // تلگرام صفحه‌کلید ثابت را فقط **سوار یک پیام** نصب می‌کند؛ راهی برای
  // نصبِ بی‌پیام وجود ندارد. تا امروز آن پیام یک خط راهنما بود
  // («منوی نارمون همیشه پایین همین صفحه…») و اولین چیزی می‌شد که کاربر
  // در /start می‌دید — یعنی پیشِ خوش‌آمد، یک جمله‌ی خدماتی.
  //
  // حالا همان پیام نصب می‌شود و بلافاصله پس از رسیدن کارت خوش‌آمد پاک
  // می‌شود. صفحه‌کلید ثابت وضعیتِ سمتِ کلاینت است و با حذفِ پیامِ حامل
  // نمی‌رود.
  //
  // ترتیب عمدی است: اول کارت خوش‌آمد می‌رسد، بعد حامل پاک می‌شود. اگر
  // حذف شکست بخورد، بدترین حالت همان رفتار قبلی است — یک پیام اضافه، نه
  // منوی ازدست‌رفته.
  //
  ...
  const carrierId = await sendKeyboard(chatId, "🌙", replyKeyboard());
  ...
  if (carrierId) await deleteMessage(chatId, carrierId);
}
